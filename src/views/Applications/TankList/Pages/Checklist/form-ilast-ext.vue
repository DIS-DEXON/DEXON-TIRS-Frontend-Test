  ...
  <div class="report-sheet checklist-sheet">
    <div class="report-container">
      <div class="sheet-header">
        ...
        <div class="title">ILAST External Inspection Checklist</div>
        <div class="docno"></div>
      </div>
    ...
    />
    <note v-if="this.isNoteOpen" :url="'/chk-ilast-ex/edit-chkilastex-note?id='" @refresh="refresh" :item="note_item" :id="note_id" @closePopup="TOGGLE_POPUP_NOTE" />
  </div>
</template>
...
export default {
  ...
  methods: {
    UPDATE_RESULT(item, new_result_desc, comment) {
      ...
      axios({
        method: "put",
        url: "chk-ilast-ex/edit-chkilastex",
        headers: {
        ...
        .then(res => {
          if (res.status == 200 && res.data) {
            console.log("==> RESULT UPDATED (external)");
          }
        })
        .catch(error => {
          console.log(error);
