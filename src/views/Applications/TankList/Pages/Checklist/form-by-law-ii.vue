<template>
  <div class="report-sheet checklist-sheet">
            ...
            <div class="form-item-value" style="grid-column:span 1;">
              ...
              <div class="item-wrapper" style>
                ...
                <v-ons-toolbar-button
                  class="item"
                  style="padding:0;width:20px"
                  @click="TOGGLE_POPUP_NOTE(item2.result[0])"
                >
    ...
    />
    <note v-if="this.isNoteOpen" :url="'/chk-by-law/edit-chkbylaw-2-note?id='" @refresh="refresh" :item="note_item" :id="note_id" @closePopup="TOGGLE_POPUP_NOTE" />
  </div>
</template>
...
.sheet-body {
  grid-template-columns: 40px 20% 40px 40px 40px 40px 40px auto !important;
  .topic-label {
    grid-template-columns: 40px 20% 40px 40px 40px 40px 40px auto !important;
  }
  .rating-option {
